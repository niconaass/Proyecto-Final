...
NOVEDAD V20:
- parte de la búsqueda económica por familias de V13;
- incorpora como RESTRICCIÓN DURA los límites espaciales obtenidos en V16,
  donde los aerogeneradores se llevan hacia el perímetro y se exige sombra eólica
  cero sobre el campo FV para las horas solares modeladas;
- para cada N_aeros, P_FV queda acotada por la máxima potencia FV compatible con
  el mejor layout perimetral V16;
- las cantidades de aeros sin layout espacial factible se eliminan de la búsqueda;
- P_BESS y N_containers siguen optimizándose económicamente;
- cada familia admitida recibe validación exacta de 20 años y las mejores se refinan;
- genera nuevamente el análisis anual completo del óptimo técnico-económico-espacial.

SIGUE SIN IMPLEMENTAR:
- reemplazo automático del BESS al llegar a EOL;
- diseño civil constructivo final de caminos, plataformas, zanjas y PCS.
...
El screening interno FV+BESS se conserva como filtro secundario, pero V20 además
impone los límites georreferenciados de V16 para la interacción eólico-FV y sombra cero.

Instalación:
    pip install pandas numpy openpyxl

Uso básico:
    python migracion_balance_anual_v20_familias_optimo_detallado.py --excel Balance_anual-v3.xlsm

Simulación de 20 años:
    python migracion_balance_anual_v20_familias_optimo_detallado.py --excel Balance_anual-v3.xlsm --simular-20
...
# =============================================================================
# V16 -> RESTRICCIONES ESPACIALES PARA V20
# =============================================================================

# Fallback exacto de la corrida V16 confirmada por el usuario.
# Sólo se usa si no se encuentra el CSV de V16.

# =============================================================================
# V19.1 -> RESTRICCIONES ESPACIALES PARA V20
# =============================================================================

PFV_V19_FALLBACK_3 = 23.3212
PFV_V19_FALLBACK_4 = 6.9307


def _buscar_csv_v19_1_por_defecto(carpeta_excel: Path) -> Path | None:
    candidatos = [
        carpeta_excel / "v19_1_restricciones_espaciales_por_familia.csv",
        Path(__file__).resolve().parent / "v19_1_restricciones_espaciales_por_familia.csv",
        Path.cwd() / "v19_1_restricciones_espaciales_por_familia.csv",
        Path.home() / "Downloads" / "v19_1_restricciones_espaciales_por_familia.csv",
        Path.home() / "Documents" / "Facultad" / "Taller integrador" / "v19_1_restricciones_espaciales_por_familia.csv",
        Path(r"C:\Users\migue\Documents\Facultad\Taller integrador\v19_1_restricciones_espaciales_por_familia.csv"),
    ]
    for p in candidatos:
        if p.exists():
            return p
...
def cargar_restricciones_espaciales_v19_1(
    ruta_csv: Path | None,
    *,
    carpeta_excel: Path,
    containers_min: int,
    containers_max: int,
    fv_max_global: float,
) -> tuple[dict[tuple[int, int], dict], Path | None, bool]:
    """
    Restricción espacial por familia (N_aeros, N_containers).

    0..2 aeros: V19/V19.1 permiten más de 30 MW FV, por lo que dentro del
    dominio económico P_FV<=30 MW no existe una restricción adicional.

    3..5 aeros: se leen del CSV V19.1. Si el archivo no aparece se utiliza
    únicamente como fallback la corrida ya confirmada:
        3 aeros -> 23.3212 MW
        4 aeros ->  6.9307 MW
        5 aeros -> no se encontró layout factible.
    """
    ruta = (
        ruta_csv.resolve()
        if ruta_csv is not None
        else _buscar_csv_v19_1_por_defecto(carpeta_excel)
    )

    limites: dict[tuple[int, int], dict] = {}

    for na in (0, 1, 2):
        for nc in range(containers_min, containers_max + 1):
            limites[(na, nc)] = {
                "factible": True,
                "pfv_max_mw": float(fv_max_global),
                "area_sombra_ha": float("nan"),
                "dmin_m": float("nan"),
                "fuente": "V19/V19.1: capacidad espacial > dominio FV",
            }
    ...
    if ruta is not None and ruta.exists():
        ...
        requeridas = {
            "N aeros",
            "N containers BESS",
            "Layout espacial encontrado",
            "P_FV máxima compacta [MW]",
        }
        faltan = requeridas.difference(df.columns)
        if faltan:
            raise ValueError(
                "El CSV V19.1 no tiene las columnas requeridas: "
                + ", ".join(sorted(faltan))
            )

        for _, r in df.iterrows():
            na = int(r["N aeros"])
            nc = int(r["N containers BESS"])
            if not (containers_min <= nc <= containers_max):
                continue

            raw = r["Layout espacial encontrado"]
            if isinstance(raw, str):
                fact = raw.strip().lower() in {"true", "1", "si", "sí", "yes"}
            else:
                fact = bool(raw) if pd.notna(raw) else False

            pfv = (
                float(r["P_FV máxima compacta [MW]"])
                if pd.notna(r["P_FV máxima compacta [MW]"])
                else 0.0
            )

            limites[(na, nc)] = {
                "factible": fact,
                "pfv_max_mw": min(float(fv_max_global), max(0.0, pfv)),
                "area_sombra_ha": (
                    float(r["Área sombra dentro predio [ha]"])
                    if "Área sombra dentro predio [ha]" in df.columns
                    and pd.notna(r.get("Área sombra dentro predio [ha]"))
                    else float("nan")
                ),
                "dmin_m": (
                    float(r["Distancia mínima aeros [m]"])
                    if "Distancia mínima aeros [m]" in df.columns
                    and pd.notna(r.get("Distancia mínima aeros [m]"))
                    else float("nan")
                ),
                "fuente": str(ruta),
            }

        return limites, ruta, False

    for nc in range(containers_min, containers_max + 1):
        limites[(3, nc)] = {
            "factible": True,
            "pfv_max_mw": min(float(fv_max_global), PFV_V19_FALLBACK_3),
            "area_sombra_ha": 47.782,
            "dmin_m": 653.50,
            "fuente": "fallback corrida V19.1 confirmada",
        }
        limites[(4, nc)] = {
            "factible": True,
            "pfv_max_mw": min(float(fv_max_global), PFV_V19_FALLBACK_4),
            "area_sombra_ha": 66.615,
            "dmin_m": 657.38,
            "fuente": "fallback corrida V19.1 confirmada",
        }
        limites[(5, nc)] = {
    ...
    return limites, None, True


def limite_fv_familia_v20(
    restricciones: dict[tuple[int, int], dict],
    na: int,
    nc: int,
    fv_max_global: float,
) -> tuple[bool, float, str]:
    info = restricciones.get((int(na), int(nc)))
    if info is None:
        return False, 0.0, "familia sin resultado espacial V19.1"
    if not bool(info.get("factible", False)):
        return False, 0.0, str(info.get("fuente", "V19.1"))
    return (
        True,
        min(float(fv_max_global), float(info.get("pfv_max_mw", 0.0))),
        str(info.get("fuente", "V19.1")),
    )


def imprimir_restricciones_v19_1(
    limites: dict[tuple[int, int], dict],
    *,
    fv_max_global: float,
    containers_min: int,
    ...
    print("\n" + "=" * 88)
    print("RESTRICCIONES ESPACIALES V19.1 INCORPORADAS EN V20")
    print("=" * 88)
    print("N_aeros | Estado      | Rango P_FV,max entre containers")
    print("-" * 88)
    ...
    for na in range(0, 6):
        ...
        facts = []
        for nc in range(containers_min, containers_max + 1):
            info = limites.get((na, nc))
            if info is None:
                continue
            facts.append(bool(info.get("factible", False)))
            if bool(info.get("factible", False)):
                vals.append(
                    min(float(fv_max_global), float(info["pfv_max_mw"]))
                )

        if not facts or not any(facts):
            print(f"{na:7d} | NO FACTIBLE | descartado")
        else:
            print(
                f"{na:7d} | factible    | "
                f"{min(vals):8.4f} .. {max(vals):8.4f} MW"
            )


def _buscar_cache_v17_por_defecto(carpeta_excel: Path) -> Path | None:
    candidatos = [
        carpeta_excel / "v17_mejor_por_familia.csv",
        Path.home() / "Documents" / "Facultad" / "Taller integrador" / "v17_mejor_por_familia.csv",
        Path(r"C:\Users\migue\Documents\Facultad\Taller integrador\v17_mejor_por_familia.csv"),
        Path.home() / "Downloads" / "v17_mejor_por_familia.csv",
    ]
    for p in candidatos:
        if p.exists():
            return p
...
def cargar_mejores_cache_v17(
    ruta: Path,
    aeros_cache: tuple[int, ...] = (0, 1, 2),
) -> list[dict]:
    """
    Extrae el mejor diseño V17 de 0, 1 y 2 aeros.
    Luego V20 vuelve a evaluarlos exactamente antes de compararlos.
    """
    df = pd.read_csv(ruta)
    if "Costo final familia [USD]" not in df.columns:
        raise ValueError(
            "El cache V17 no contiene la columna 'Costo final familia [USD]'."
        )

    out = []
    for na in aeros_cache:
        d = df[df["N aeros"].astype(int) == int(na)].copy()

        if "Factible exacto" in d.columns:
            f = d["Factible exacto"]
            if f.dtype == object:
                mask = f.astype(str).str.lower().isin(
                    ["true", "1", "si", "sí", "yes"]
                )
            else:
                mask = f.fillna(False).astype(bool)
            d = d[mask]

        d["__costo"] = pd.to_numeric(
            d["Costo final familia [USD]"],
            errors="coerce",
        )
        d = d[d["__costo"].notna()]
        if d.empty:
        ...
        r = d.loc[d["__costo"].idxmin()]

        def val(refinado: str, inicial: str) -> float:
            if refinado in d.columns and pd.notna(r.get(refinado)):
                return float(r[refinado])
            return float(r[inicial])

        out.append({
            "N aeros": int(na),
            "N containers BESS": int(r["N containers BESS"]),
            "P_FV [MW]": val(
                "P_FV refinado [MW]",
                "P_FV exacto inicial [MW]",
            ),
            "P_BESS [MW]": val(
                "P_BESS refinado [MW]",
                "P_BESS exacto inicial [MW]",
            ),
            "Costo cache V17 [USD]": float(r["__costo"]),
        })
...
# =============================================================================
# V20 - OPTIMIZACIÓN POR FAMILIAS DISCRETAS + INFORME FINAL DETALLADO
# =============================================================================

def _json_safe_v20(v):
...
def _evaluar_exacto_subproceso_v20(
    *,
    ruta_excel: Path,
    pfv: float,
    na: int,
...
def _candidatos_diversos_familia_v20(explorados: list[dict], cantidad: int = 3) -> list[dict]:
...
def _explorar_familia_surrogate_v20(
    perfiles: pd.DataFrame,
    *,
    na: int,
    nc: int,
    ...
    V20 no usa los rangos encontrados por V12 como límites.
    """
    na, nc = int(na), int(nc)
    ...
    return _candidatos_diversos_familia_v20(explorados, candidatos_guardar), res


def _refinar_familia_exacto_v20(
    *,
    mejor_inicial: dict,
    na: int,
    nc: int,
    ...
    for ronda in range(max(0, int(rondas))):
        ...
        for pfv, pb in puntos:
            ...
            if abs(pfv - pf0) < 1e-9 and abs(pb - pb0) < 1e-9:
                continue
            contador += 1
            ev = _evaluar_exacto_subproceso_v20(
            ...
            if ev is None:
                continue
            rr, d20, dh1, plan = ev
            rr["Etapa V20"] = f"refinamiento ronda {ronda+1}"
            hist.append(dict(rr))
...
def optimizar_por_familias_v20(
    perfiles: pd.DataFrame,
    *,
    cfg_base: Configuracion,
    ruta_excel: Path,
    ...
    restricciones_espaciales_v19_1: dict[tuple[int, int], dict],
    ...
    familias: list[tuple[int, int]] = []
    descartadas: list[tuple[int, int]] = []

    for na in range(aeros_min, aeros_max + 1):
        for nc in range(containers_min, containers_max + 1):
            fact, cap, _ = limite_fv_familia_v20(
                restricciones_espaciales_v19_1,
                na, nc, fv_max,
            )
            if (not fact) or cap < float(fv_min) - 1e-12:
                descartadas.append((na, nc))
                continue
            familias.append((na, nc))

    print("\n" + "=" * 88)
    print("V20 - REOPTIMIZACIÓN ECONÓMICA CON RESTRICCIÓN ESPACIAL V19.1")
    print("=" * 88)
    print(f"Familias efectivamente reoptimizadas : {len(familias)}")
    print(f"Rango de aeros reoptimizado           : {aeros_min} .. {aeros_max}")
    print(f"P_FV global                           : [{fv_min:g}, {fv_max:g}] MW")
    print(f"P_BESS                                : [{pbess_min:g}, {pbess_max:g}] MW, además P<=0,5E")
    print("Restricción espacial                  : P_FV <= P_FV,max(N_aeros,N_containers)")
    print("Fuente espacial                       : V19.1 / sombra V18 / FV compacta")
    print("Validación                            : modelo exacto 20 años por familia")
    print("Objetivo                              : mínimo costo total descontado a 20 años")
    if descartadas:
        print(f"Familias descartadas por espacio      : {len(descartadas)}")
    ...
    with tempfile.TemporaryDirectory(prefix="v20_familias_") as td:
        td = Path(td)
        for idx, (na, nc) in enumerate(familias, 1):
            fact_esp, fv_max_familia, fuente_limite = limite_fv_familia_v20(
                restricciones_espaciales_v19_1,
                na, nc, fv_max,
            )
            if not fact_esp:
            ...
            print(
                f"\nFamilia {idx}/{len(familias)}: aeros={na}, containers={nc} "
                f"(E={nc*cfg_base.e_container_mwh:.3f} MWh) | "
                f"FV <= {fv_max_familia:.4f} MW [{fuente_limite}]"
            )

            cands, _ = _explorar_familia_surrogate_v20(
                perfiles, na=na, nc=nc, cfg_base=cfg_base,
                fv_min=fv_min, fv_max=fv_max_familia, pbess_min=pbess_min, pbess_max=pbess_max,
            ...
            if not cands:
                filas_familias.append({
                    ...
                    "P_FV máximo espacial V19.1 [MW]": fv_max_familia,
            ...
            for j, cand in enumerate(cands[:max(1, int(exactos_por_familia))], 1):
                ...
                rr["Etapa V20"] = "validación familia"
        ...
        if not exactos_familia:
            raise RuntimeError("Ninguna familia resultó factible en validación exacta V20.")
        ...
        for k, ini in enumerate(top, 1):
            ...
            _, fv_max_refinar, _ = limite_fv_familia_v20(
                restricciones_espaciales_v19_1,
                na, nc, fv_max,
            )
            actual, d20, dh1, plan, hist, contador_exacto = _refinar_familia_exacto_v20(
                mejor_inicial=dict(ini), na=na, nc=nc, cfg_base=cfg_base,
                ruta_excel=ruta_excel, fv_min=fv_min, fv_max=fv_max_refinar,
...
def construir_evolucion_anual_v20(detalle20: pd.DataFrame, mejor: dict) -> pd.DataFrame:
...
def construir_resumen_economico_v20(detalle20: pd.DataFrame, mejor: dict) -> pd.DataFrame:
...
def generar_analisis_final_v20(mejor: dict, evolucion: pd.DataFrame, economico: pd.DataFrame) -> str:
    ...
    lines = [
        "TRABAJO INTEGRADOR - ANÁLISIS FINAL DEL ÓPTIMO V20",
        "="*72,
        "",
        "DISEÑO ÓPTIMO",
    ...
    lines += [
        ...
        "única tecnología. La restricción espacial proviene del prediseño V19.1 (sombra V18 + FV compacta). No constituye todavía un layout constructivo con caminos, zanjas y obra civil final.",
        "",
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="V20: optimización económica final con límites espaciales V19.1 y comparación exacta contra V17 para 0..2 aeros."
    )
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="CSV v19_1_restricciones_espaciales_por_familia.csv. Se busca automáticamente.",
    )
    parser.add_argument(
        "--cache-v17",
        type=Path,
        default=None,
        help="CSV v17_mejor_por_familia.csv para reutilizar 0..2 aeros.",
    )
    parser.add_argument(
        "--sin-cache-v17",
        action="store_true",
        help="Optimiza sólo el rango indicado y no compara con V17.",
    )

    parser.add_argument("--optimizar", action="store_true", help="Ejecuta V20 con restricciones V19.1.")
    ...
    parser.add_argument("--aeros-min", type=int, default=3)
    parser.add_argument("--aeros-max", type=int, default=4)
    ...
    # Los subprocess exactos reciben una configuración ya acotada.
    restricciones_v19_1 = None
    ruta_v19 = None
    uso_fallback_v19 = False
    ruta_cache_v17 = None
    cache_v17 = []

    if not args.evaluar_candidato_interno:
        restricciones_v19_1, ruta_v19, uso_fallback_v19 = cargar_restricciones_espaciales_v19_1(
            args.espacio_v19,
            carpeta_excel=carpeta,
            containers_min=args.containers_min,
            containers_max=args.containers_max,
            fv_max_global=args.fv_max,
        )

        if ruta_v19 is not None:
            print(f"Restricción espacial V19.1: {ruta_v19}")
        else:
            print("Restricción espacial V19.1: usando fallback de la corrida confirmada.")

        imprimir_restricciones_v19_1(
            restricciones_v19_1,
            fv_max_global=args.fv_max,
            containers_min=args.containers_min,
            containers_max=args.containers_max,
        ...
        if not args.sin_cache_v17:
            ruta_cache_v17 = (
                args.cache_v17.resolve()
                if args.cache_v17 is not None
                else _buscar_cache_v17_por_defecto(carpeta)
            )
            if ruta_cache_v17 is not None and ruta_cache_v17.exists():
                cache_v17 = cargar_mejores_cache_v17(ruta_cache_v17)
                print(f"Cache económico V17: {ruta_cache_v17}")
                print(f"Candidatos V17 de 0..2 aeros recuperados: {len(cache_v17)}")
            else:
                print(
                    "AVISO: no encontré v17_mejor_por_familia.csv. "
                    "Si querés comparación global, indicá --cache-v17 o corré "
                    "--aeros-min 0 --aeros-max 4 --sin-cache-v17."
                )
    ...
    if args.evaluar_candidato_interno:
        ...
        Path(str(pref)+"_resumen.json").write_text(
            json.dumps({k:_json_safe_v20(v) for k,v in rr.items()}, ensure_ascii=False, indent=2), encoding="utf-8"
        )
    ...
    if not args.optimizar:
        print("\nV20 está pensada para buscar el óptimo. Ejecutá el archivo con --optimizar.")
        print("No se evalúa automáticamente la configuración del Excel para no perder tiempo.")
        return

    mejor, familias, hist, detalle20, detalle_h1, plan = optimizar_por_familias_v20(
        perfiles,
        ...
        restricciones_espaciales_v19_1=restricciones_v19_1,
    )

    # ---------------------------------------------------------------------
    # Comparación exacta con las mejores soluciones V17 de 0..2 aeros.
    # V19/V19.1 no cambia su dominio espacial dentro de P_FV<=30 MW.
    # ---------------------------------------------------------------------
    comparacion_cache_rows: list[dict] = []

    if cache_v17:
        print("\n" + "-" * 88)
        print("COMPARACIÓN EXACTA CONTRA MEJORES V17 DE 0..2 AEROS")
        print("-" * 88)

        with tempfile.TemporaryDirectory(prefix="v20_cache_v17_") as td_cache:
            td_cache = Path(td_cache)

            for j, c in enumerate(cache_v17, 1):
                ev = _evaluar_exacto_subproceso_v20(
                    ruta_excel=ruta,
                    pfv=float(c["P_FV [MW]"]),
                    na=int(c["N aeros"]),
                    pbess=float(c["P_BESS [MW]"]),
                    nc=int(c["N containers BESS"]),
                    tipo_aero=args.tipo_aero,
                    potencia_modulo_fv_w=args.pot_modulo_fv_w,
                    pitch_fv_m=args.pitch_fv_m,
                    exportar_excedente=not args.sin_exportar,
                    carpeta_tmp=td_cache,
                    etiqueta=f"cache_v17_{j}",
                )

                if ev is None:
                    print(
                        f"  aeros={c['N aeros']}: no pudo reevaluarse exactamente."
                    )
                    continue

                rr_c, d20_c, dh1_c, plan_c = ev
                rr_c["Etapa V20"] = "comparación cache V17 exacta"

                comparacion_cache_rows.append({
                    "Fuente": "cache V17 reevaluado exacto",
                    "N aeros": int(rr_c["N aeros"]),
                    "N containers BESS": int(rr_c["N containers BESS"]),
                    "P_FV [MW]": float(rr_c["P_FV [MW]"]),
                    "P_BESS [MW]": float(rr_c["P_BESS [MW]"]),
                    "Costo total 20a [USD]": float(rr_c["Costo total 20a [USD]"]),
                    "Costo cache V17 informado [USD]": float(c["Costo cache V17 [USD]"]),
                })

                print(
                    f"  aeros={int(rr_c['N aeros'])} | "
                    f"containers={int(rr_c['N containers BESS'])} | "
                    f"costo=${float(rr_c['Costo total 20a [USD]']):,.0f}"
                )

                if float(rr_c["Costo total 20a [USD]"]) + 1e-6 < float(mejor["Costo total 20a [USD]"]):
                    mejor = dict(rr_c)
                    detalle20 = d20_c
                    detalle_h1 = dh1_c
                    plan = plan_c

    if comparacion_cache_rows:
        df_cache = pd.DataFrame(comparacion_cache_rows)
        df_cache["Factible exacto"] = True
        df_cache["Costo final familia [USD]"] = df_cache["Costo total 20a [USD]"]
        df_cache["P_FV exacto inicial [MW]"] = df_cache["P_FV [MW]"]
        df_cache["P_BESS exacto inicial [MW]"] = df_cache["P_BESS [MW]"]
        df_cache["Origen V20"] = "reutilizado V17; espacio sin cambios"

        familias["Origen V20"] = "reoptimizado con V19.1"
        familias = pd.concat(
            [familias, df_cache],
            ignore_index=True,
            sort=False,
        )
        familias = familias.sort_values(
            "Costo final familia [USD]",
            na_position="last",
        ).reset_index(drop=True)

    n_opt = int(mejor["N aeros"])
    nc_opt = int(mejor["N containers BESS"])

    fact_opt, cap_esp_opt, fuente_esp_opt = limite_fv_familia_v20(
        restricciones_v19_1,
        n_opt, nc_opt, args.fv_max,
    )

    mejor["Layout espacial V19.1 factible"] = bool(fact_opt)
    mejor["P_FV máximo espacial V19.1 [MW]"] = float(cap_esp_opt)
    mejor["Margen FV hasta límite espacial [MW]"] = (
        float(cap_esp_opt) - float(mejor["P_FV [MW]"])
    )
    mejor["Fuente restricción espacial"] = fuente_esp_opt

    evolucion = construir_evolucion_anual_v20(detalle20, mejor)
    economico = construir_resumen_economico_v20(detalle20, mejor)
    analisis = generar_analisis_final_v20(mejor, evolucion, economico)

    print("\n" + "="*80)
    print("MEJOR SOLUCIÓN ENCONTRADA V20 - ECONÓMICA + ESPACIAL V19.1")
    print("="*80)
    for k, v in mejor.items():
        if isinstance(v, (float, np.floating)):
    ...
    # Archivos finales V20.
    f_fam = carpeta / "v20_mejor_por_familia.csv"
    f_hist = carpeta / "v20_evaluaciones_exactas.csv"
    f_evol = carpeta / "optimo_v20_evolucion_20_anios.csv"
    f_eco = carpeta / "optimo_v20_desglose_economico.csv"
    f_plan = carpeta / "optimo_v20_plan_degradacion.csv"
    f_h1 = carpeta / "optimo_v20_anio1_horario.csv"
    f_txt = carpeta / "optimo_v20_analisis_final.txt"
    f_json = carpeta / "optimo_v20_resumen.json"
    f_esp = carpeta / "v20_restricciones_espaciales_v19_1_usadas.csv"
    f_cache = carpeta / "v20_comparacion_cache_v17.csv"

    filas_esp = []
    for (na, nc), info in sorted(restricciones_v19_1.items()):
        filas_esp.append({
            "N aeros": int(na),
            "N containers BESS": int(nc),
            "Layout espacial factible": bool(info["factible"]),
            "P_FV máxima espacial usada [MW]": (
                min(float(args.fv_max), float(info["pfv_max_mw"]))
                if bool(info["factible"]) else 0.0
            ),
            "Área sombra V19.1 [ha]": float(
                info.get("area_sombra_ha", float("nan"))
            ),
            "Distancia mínima layout [m]": float(
                info.get("dmin_m", float("nan"))
            ),
            "Fuente": str(info.get("fuente", "")),
        })

    pd.DataFrame(filas_esp).to_csv(
        f_esp,
        index=False,
        decimal=".",
    )
    ...
    if comparacion_cache_rows:
        pd.DataFrame(comparacion_cache_rows).to_csv(
            f_cache,
            index=False,
            decimal=".",
        )
    ...
    f_json.write_text(json.dumps({k:_json_safe_v20(v) for k,v in mejor.items()}, ensure_ascii=False, indent=2), encoding="utf-8")
